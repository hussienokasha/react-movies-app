...
import type { TvVideo } from "../types/tv.types";
import { getTvVideos } from "../api/tv.api";
...
export const dialog = createOverlayManager<DialogPayload>(
  ({ title, id, ...rest }) => {
    const [tvVideos, setTvVideos] = useState<TvVideo[]>([]);
    useEffect(() => {
    ...
    const trailerKey = officialTrailer?.key ?? tvVideos[0]?.key;
                ...
                {tvVideos.length > 0 && trailerKey ? (
                  ...
                  <div className="flex aspect-video items-center justify-center bg-black/5 p-6 text-center">
                    <p className="text-sm text-gray-500">
                      No trailer available for this tv.
                    </p>
                  </div>
                )}
              </Dialog.Content>
